...
const AXES = [
  ...
  { key: 'vax_rate',           label: 'Vax Rate',        max: 100,    invert: false },
    ...
    >
      ...
      {rings.map(r => {
        const pts = AXES
          ...
          .map(([x, y], i) => `${i === 0 ? 'M' : 'L'} ${x.toFixed(1)},${y.toFixed(1)}`)
          .join(' ') + ' Z';
        return <path key={r} d={pts} fill="none" stroke="rgba(79,126,255,.12)" strokeWidth="1" />;
      })}
      ...
      {AXES.map((_, i) => {
        ...
        return (
          <line key={i}
            ...
            stroke="rgba(79,126,255,.18)" strokeWidth="1"
          />
        );
      })}

      {/* Data polygon */}
      <path d={dataPath} fill="rgba(79,126,255,.13)" stroke="rgba(79,126,255,.88)" strokeWidth="1.8" />

      {/* Dots */}
      {dataPoints.map(([x, y], i) => (
        <circle key={i} cx={x.toFixed(1)} cy={y.toFixed(1)} r="3.5" fill="#4f7eff" />
      ))}
