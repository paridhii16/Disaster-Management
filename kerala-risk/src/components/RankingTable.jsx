...
export default function RankingTable({ districts, onSelect }) {
  const sorted = [...districts].sort((a, b) => b.vulnerability - a.vulnerability);
  ...
  return (
    <div className="card" style={{ flex: 1, padding: '18px 20px' }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        <thead>
          <tr>
            {['#', 'District', 'Score', 'Beds/1k', 'Literacy', 'Density', 'Risk'].map(h => (
              <th key={h} style={{
                color: 'var(--muted)', fontWeight: 400, textAlign: 'left',
                padding: '5px 6px 9px', fontSize: 11.5,
          ...
          {sorted.map((d, i) => {
            ...
            return (
              <tr
                ...
                onMouseEnter={e => e.currentTarget.style.background = 'rgba(79,126,255,.06)'}
                onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
              >
                <td style={{ padding: '9px 6px', color: 'var(--faint)', fontSize: 12 }}>{i + 1}</td>
                <td style={{ padding: '9px 6px', fontWeight: 500 }}>{d.district}</td>
                <td style={{
                  padding: '9px 6px',
