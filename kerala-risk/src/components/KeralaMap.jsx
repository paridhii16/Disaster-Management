...
const METRIC_OPTIONS = [
  { value: 'vulnerability',   label: 'Vulnerability Score',  max: 100,    higherBad: true  },
  { value: 'density',         label: 'Population Density',   max: 1600,   higherBad: true  },
  { value: 'beds_per_1000',   label: 'Hospital Beds / 1k',   max: 2,      higherBad: false },
  { value: 'gddp_per_capita', label: 'GDDP per Capita (₹)',  max: 500000, higherBad: false },
  { value: 'literacy_rate',   label: 'Literacy Rate',        max: 100,    higherBad: false },
  { value: 'vax_rate',        label: 'Vaccination Rate',     max: 100,    higherBad: false },
];
      ...
      >
        ...
        {geoLoading && (
          <div style={{
            ...
            <svg width="38" height="38" viewBox="0 0 38 38">
              <circle cx="19" cy="19" r="14" fill="none" stroke="var(--border2)" strokeWidth="3" />
              <circle cx="19" cy="19" r="14" fill="none" stroke="var(--accent)" strokeWidth="3"
                strokeDasharray="20 66" strokeLinecap="round" transform="rotate(-90 19 19)">
                <animateTransform attributeName="transform" type="rotate"
                  from="-90 19 19" to="270 19 19" dur="0.9s" repeatCount="indefinite" />
              </circle>
            </svg>
        ...
        {geoError && (
          <div style={{
            ...
            color: 'var(--muted)', padding: 24, textAlign: 'center',
          }}>
            <div style={{ fontSize: 26, color: 'var(--red)' }}>⚠</div>
            <div style={{ fontSize: 13, color: 'var(--text)' }}>Could not load GeoJSON</div>
        ...
        {!geoLoading && !geoError && (
          ...
          >
            <text x="20" y="22" fill="rgba(79,126,255,.45)" fontSize="11"
              ...
              {pathData.map(item => {
                ...
                return (
                  <g key={item.csvName + '_lbl'}>
                    ...
                    {d && (
                      <text
                        x={item.cx} y={item.cy + 11}
                        textAnchor="middle" dominantBaseline="central"
                        fill="rgba(255,255,255,.55)" fontSize="8"
        ...
        {tooltip && (
          <div style={{
            ...
            background: 'rgba(6,11,22,.97)', border: '1px solid var(--border2)',
            borderRadius: 10, padding: '12px 15px', fontSize: 12.5,
            pointerEvents: 'none', zIndex: 99, minWidth: 190,
            boxShadow: '0 10px 28px rgba(0,0,0,.55)',
          }}>
            <div style={{
              fontFamily: 'var(--head)', fontSize: 15,
              fontWeight: 600, marginBottom: 8,
            }}>
              {tooltip.d.district}
            </div>
            {[
              ['Vulnerability', `${tooltip.d.vulnerability}/100`, riskColor(tooltip.d.vulnerability)],
              ['Population',    `${(tooltip.d.population / 1e6).toFixed(2)}M`,  null],
              ['Density',       `${tooltip.d.density}/km²`,                     null],
              ['Beds / 1k',     tooltip.d.beds_per_1000,                        null],
              ['Literacy',      `${tooltip.d.literacy_rate}%`,                  null],
              ['Vax rate',      `${tooltip.d.vax_rate}%`,                       null],
            ].map(([lbl, val, col]) => (
              <div key={lbl} style={{
                display: 'flex', justifyContent: 'space-between',
                gap: 14, color: 'var(--muted)', marginBottom: 3,
              }}>
                <span>{lbl}</span>
                <span style={{ color: col || 'var(--text)', fontWeight: 500 }}>{val}</span>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={{
          ...
          background: 'rgba(6,11,22,.88)', border: '1px solid var(--border)',
          borderRadius: 9, padding: '10px 13px',
        }}>
          <div style={{
            fontSize: 10.5, color: 'var(--muted)', marginBottom: 7,
          ...
          {[
            ...
            <div key={label} style={{
              ...
              gap: 7, marginBottom: 4, fontSize: 11.5,
            }}>
              <div style={{ width: 14, height: 9, borderRadius: 2, background: color }} />
              <span>{label}</span>
            </div>
          ))}
        </div>
